...
async function readAll() {
  ...
  const sql = `
     SELECT 
      m.id_meliponario, m.nome, m.descricao, m.qtd_caixas, m.telefone, m.image, m.bairro, m.rua, m.numero, r.UF as regiao
    FROM 
...
async function create(meliponario) {
  ...
  const { nome, descricao, qtd_caixas, telefone, image, bairro, rua, numero, regioes_id } = meliponario;
  const id_meliponicultor = await Meliponicultores.infoToken(token);
  const sql = `
    INSERT INTO
      Meliponarios (nome, descricao, qtd_caixas, telefone, image, bairro, rua, numero, regioes_id, id_meliponicultor)
    VALUES
      (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
  `;
  
  const {lastID} = await db.run(sql, [nome, descricao, qtd_caixas, telefone, image, bairro, rua, numero, regioes_id, id_meliponicultor]);
...
async function update(meliponario, id_meliponario) {
  ...
  const { nome, qtd_caixas, telefone, image, bairro, rua, numero, descricao, regioes_id } = meliponario;

  const sql = `
    UPDATE
      Meliponarios
    SET
      nome = ?, qtd_caixas = ?, telefone = ?, image = ?,  bairro = ?, rua = ?, numero = ?, descricao = ?, regioes_id = ?
    WHERE
      id_meliponario = ?
  `;

  const { changes } = await db.run(sql, [nome, qtd_caixas, telefone, image, bairro, rua, numero, descricao, regioes_id, id_meliponario]);
